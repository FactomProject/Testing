#script to transfer funds to multiple factoid addresses from a single address from cli
...
#remove the transaction name just in case transaction fails. if transaction succeeds then this command will fail.
factom-cli rmtx t1
sleep 1s
done
